...
export default class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    // Grupo para plataformas estáticas (Chão)
    this.platforms = this.physics.add.staticGroup();
    // Grupo para plataformas que se movem
    this.movingPlatforms = this.physics.add.group({
      allowGravity: false,
      immovable: true,
    });
    // Grupo para plataformas que caem
    this.fallingPlatforms = this.physics.add.group({
      allowGravity: false,
      immovable: true,
    });
    // 3. ATUALIZE O LOOP DE CRIAÇÃO
    levelData.platforms.forEach((p) => {
      if (p.type === "moving") {
        ...
        // Plataforma estática padrão (Chão)
        this.platforms
    ...
    if (levelData.saws) {
      levelData.saws.forEach((s) => {
        // A própria classe Saw agora cuida de sua configuração
        ...
        newSaw.body.setAngularVelocity(150); // Faz a serra girar
      });
    }
    ...
    // O callback é chamado quando o jogador colide com uma plataforma que cai
    this.physics.add.collider(
      this.player,
  ...
  triggerPlatformFall(player, platform) {
    // A colisão só deve ser acionada se o jogador estiver em cima da plataforma
  ...
  goToNextLevel() {
    // Modificada para não precisar do 'goal'
